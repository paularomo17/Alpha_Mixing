...
def visualise_setting_path(setting_path, exp_list, exp_alternate_names, max_rounds=-1, vis_changes=False):
    ...
    exp_preds = np.zeros([len(exp_list), reps, len(rounds)], dtype=np.string)
    for exp_name in os.listdir(setting_path):
        ...
        preds_reader = csv.reader(open(exp, 'r'), quoting=csv.QUOTE_ALL)
        accs, times, changes = [], [], []
        preds = []
    ...
    exp_preds = np.array(exp_preds, dtype=np.string)
