...
def get_MNIST(data_dir):
    raw_tr = datasets.MNIST(os.path.join(data_dir, 'MNIST'), train=True, download=True, transform=transforms.
                            Compose([transforms.Resize(32), transforms.ToTensor()]))
    raw_te = datasets.MNIST(os.path.join(data_dir, 'MNIST'), train=False, download=True)
    index_tr = random.sample(range(60000), 3000)
    index_te = random.sample(range(10000), 500)
    # X_tr = Subset(raw_tr, index_tr)
    # Y_tr = Subset(raw_tr, index_tr)
    # X_tr_or = raw_tr.data
    X_tr = raw_tr.data[index_tr]
    Y_tr = raw_tr.targets[index_tr]
    X_te = raw_te.data[index_te]
    Y_te = raw_te.targets[index_te]
